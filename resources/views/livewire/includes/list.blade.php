    ...
    <div wire:key="{{$todo->id}}" class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
        <div class="flex justify-between space-x-2">
            <div class="flex items-center">
                @if ($todo->completed)
                <input wire:click="toggle({{$todo->id}})" type="checkbox" class="form-checkbox h-4 w-4 bg-green-600 text-teal-600 mr-2" checked>
                    @else
                    <input wire:click="toggle({{$todo->id}})" type="checkbox" class="form-checkbox h-4 w-4 text-teal-600 mr-2">
                @endif
        ...
        <span class="text-xs text-gray-500"> {{$todo->created_at->format('d/m/Y - h:m')}} </span>
